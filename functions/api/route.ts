...
import { planRoute, When } from "../../src/journey";

const HOME = "51.5611161,-0.0739865";

// /api/route?from=<lat>,<lon>&to=<lat>,<lon>&toName=<label>&stage=fast|full
//   &at=<epoch ms>&timeIs=depart|arrive   -> plan for a specific time instead of now
...
export const onRequestGet: PagesFunction<Env> = async ({ env, request }) => {
  ...
  // Optional scheduled time. Ignore a departure time in the past (or unparseable) -> "now".
  const at = Number(u.searchParams.get("at"));
  const arriving = u.searchParams.get("timeIs") === "arrive";
  const when: When | null = Number.isFinite(at) && at > 0 && (arriving || at > Date.now() - 60000)
    ? { ms: at, arriving } : null;
  ...
  const result = await planRoute(env, from[0], from[1], to[0], to[1], stage, toName, mode, when);
